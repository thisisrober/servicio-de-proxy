...
#Este es el script final donde cumple todas las condiciones que se solicitan en el proyecto.
...
function instalarConDocker() {
    ...
    if command -v docker &> /dev/null; then
        if [ -f "Dockerfile" ]; then
            docker build -t ubuntu-proxy:latest -f Dockerfile .
            if docker images | grep -q "ubuntu-proxy"; then
                echo "Imagen ubuntu-proxy construida correctamente."
                docker run -d --name squid_proxy ubuntu-proxy:latest
                echo "Contenedor Squid en ejecución."
                docker ps --filter "name=squid_proxy"
            else
                echo "Hubo un problema al crear la imagen de Docker."
            fi
        else
        ...
        if groups $USERNAME | grep &>/dev/null "\bdocker\b"; then
            echo "El usuario $USERNAME ya está en el grupo docker."
        else
            echo "El usuario $USERNAME no está en el grupo docker. Añadiéndolo..."
            sudo usermod -aG docker $USERNAME
            newgrp docker
            echo "Usuario $USERNAME añadido al grupo docker. Ahora tiene permisos para usar Docker sin sudo."
        fi
        docker build -t ubuntu-proxy:latest -f Dockerfile .
        docker run -d --name squid_proxy ubuntu-proxy:latest
        echo "Contenedor Squid en ejecución."
